...
class PlanningGraph():
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        ## Verification if positive effects of node1 action do not conflict with node2 negative preconditions
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.precond_neg:
                return True
        ## Verification if positive effects of node2 action do not negate the node1 - preconditions  
        for effect in node_a2.action.effect_add:
            if effect in node_a1.action.precond_neg:
                return True
                
        ## Verification if effects of one node do not remove + preconditions of the other    
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        return all(parent.is_mutex(parent2) for parent in node_s1.parents for parent2 in node_s2.parents)
    ...
    def h_levelsum(self) -> int:
        ...
        goal_expressions = set(PgNode_s(goal, True) for goal in self.problem.goal)
        ...
        for level_number,s_level in enumerate(self.s_levels):
            for node_s in s_level:
                if node_s in goal_expressions:
                    ...
                    goal_expressions.remove(node_s)
